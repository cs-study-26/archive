...
>    [m/2]는 m/2를 반올림한 값  
>    ➡️ 각 노드(리프 노드 포함)의 원소 수는 최소 `[m/2] - 1`개 ~ 최대 `m - 1`개를 가진다.
> 4. 모든 리프 노드들은 같은 레벨에 있어야 한다.
> 5. 리프가 아닌 모든 노드들(즉, 루트 노드)은 최소 2개 이상의 자식을 가져야 한다.
...
>    2. 만일 인접 형제 노드가 새로 구성되더라도 원래 k의 부모 노드가 최소 key의 개수보다 작아진다면, 부모 노드에 대하여 2번 과정부터 다시 수행한다(3-3-1 과정을 수행하고 보니 B-Tree 구조가 깨진 경우를 말한다).
> 
...
> [!NOTE]
> M차 B-Tree의 높이는 `log_M(N)`이다.
> 
> ![b_tree_height.png](res/b_tree_height.png)  
...
> [!NOTE]
> B+Tree의 h 도출 과정  
> 최악의 경우(높이 최대) = 각 노드가 최소한의 자식만 가질 때
> 
> Level 0 (Root)  :  1개
> Level 1         :  최소 2개  
> Level 2         :  최소 2 · ⌈m/2⌉개  
> Level 3         :  최소 2 · ⌈m/2⌉²개  
> ...  
> Level h (Leaf)  :  최소 2 · ⌈m/2⌉^(h-1)개
> 
> 각 리프 노드는 최소 `[m/2] - 1`개의 키를 가지므로 N개의 키에 대해 다음과 같은 식이 성립한다.  
> ➡️ `N ≥ 2 · ⌈m/2⌉^(h-1) · (⌈m/2⌉ - 1)`  
> 
> k = ⌈m/2⌉로 놓으면:  
> N ≥ 2(k-1) · k^(h-1)  
...
| 중복 키  |트리에 중복 키가 없다.| 중복 키가 존재하며, 모든 데이터들은 리프에 있다.                                             |
